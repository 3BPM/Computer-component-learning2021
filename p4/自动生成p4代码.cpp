...
int is_small[40],small[40],val[40],cnt,cnt2;
...
void print(int x)
{
	...
	if(a[x].type==7)
		if(a[x].r3<999999) 
			cout<<"beq $"<<a[x].r1<<",$"<<a[x].r2<<",branch"<<a[x].r3<<endl;
		else
			cout<<"beq $0,$0,end"<<endl;
	if(a[x].type==8) cout<<"jal func"<<a[x].r3<<endl;
	if(a[x].type==9) cout<<"jr $ra"<<endl;
}
int main()
{
	//0nop 1addu 2subu 3lui 4ori 5lw 6sw 7beq 8jal 9jr 
	int i,j;
	srand(time(0));
	size_im=512,size_dm=1024,size_op=9,small_reg=8;		
	get_small();
	for(i=1;i<=small_reg;i++)
		a[i]=(op(4,small[i],0,val[small[i]]=r(0,size_dm-1,0)));
	for(i=size_im*3/4+size_im/16;i<=size_im;i+=size_im/16)
	{
		a[i]=op(9,0,0,0);
		branch[i-size_im/16].push_back(--cnt2);
	}
	a[size_im*3/4-1]=op(7,0,0,1000000);
	for(i=small_reg+1;i<=size_im;i++)
	{
		int op0=r(0,size_op-1,0),r1,r2,r3;
		if(a[i].type!=0) continue;
		if(i>=size_im*3/4) op0=r(0,6,0);
	...
	for(i=1;i<=size_im;i++)
	{
		...
		{
			if(branch[i][j]>0) cout<<"branch"<<branch[i][j]<<":"<<endl;
			else cout<<"func"<<-branch[i][j]<<":"<<endl;
		}
	}
	cout<<"end:";
}
